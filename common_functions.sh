...
target_group_names() {
  ...
  # This operates under the assumption that instances are only ever part of a single ASG.
  local target_groups=$($AWS_CLI autoscaling describe-load-balancer-target-groups\
  --auto-scaling-group-name $(autoscaling_group_name $instance_id) --query LoadBalancerTargetGroups[*].LoadBalancerTargetGroupARN\
  --output=text)

  if [ $? != 0 ]; then
    return 1
  elif [ "$target_groups" == "None" ]; then
    echo ""
  else
    echo "${target_groups}"
  fi
...
get_instance_environment() {
    local instance_id=$1
    $AWS_CLI ec2 describe-tags --filters Name=resource-id,Values=$instance_id Name=key,Values=Environment --query Tags[0].Value --output text
    return $?
}
